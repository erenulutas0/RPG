...
The first stone/rim material and actor-contact pass is implemented and fully automated-tested: `../stone-contact-01/` records actual Unity renders and its limited scope. Its APK is installed and observed in live S23 combat; controlled rim drags and ten-enemy warm restarts for this build await device-control coordination (docs/23). The owner's active run was preserved at its choice.

Current gaps are separate normal/boss geometry, final sidewall/prop/actor artwork, overlapping pursuit spacing, the hero's missing blocking body, and weapon balance while kiting. The next artwork proof should pair the hero with an ember Mite before scaling the roster. Pause-menu polish is deferred behind this combat-scene priority, not forgotten. Seeded offers and Forge Pulse remain deferred by the owner; neither was implemented in these art/performance slices.
