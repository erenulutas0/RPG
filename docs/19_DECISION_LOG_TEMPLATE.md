...
- **Packs:** `EntrySides` spreads each wave's slots round the four corners, one corner further per wave, 5 floor units from the hero; `PackMotion` walks each enemy to its own point on an arc round the hero and follows the hero when it moves. Amended the same day after a read-through: a corner whose enemies would start off the platform, from where the hero stands, closes for that wave and the open corners take its enemies, and an enemy whose point beside the hero hangs over the void turns round the hero to the nearest point on the platform. Packs used to enter round the arena's centre, so a hero at the rim was entered behind or beside, and enemies could stop over the void.
