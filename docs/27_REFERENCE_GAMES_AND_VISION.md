...
| S2b | **The cards themselves**: author them, widen the pool, rarity tiers per card, Luck, measure distributions across seeds and re-pin what moves | a run's build differs from the next one's, with the new baseline measured rather than tuned | S2a |
