...
7. The line under the title reads **Ember Halls | Room 1/6 | Ember Hall**, the status **Wave 1/2: combat is automatic**, with **Hits this fight** counting up and **Level 0 | XP 0 / 10**. Enemies are spawned at runtime from their prefabs; the Hierarchy shows the current one by name.
8. On the fifth hit the Grunt shape disappears, XP reaches 10, the screen dims and **Level up! Choose one upgrade** shows **Tempered Edge** (+5 damage per hit) and **Quickened Grip** (+50% attack speed). Behind the overlay the status reads **Grunt defeated in 5 hits, 3.2s**. Combat is paused and nothing advances while the panel is open.
...
13. Killing the Warden ends the floor at once: **Floor cleared**, **Vanguard defeated the Forge Warden and cleared Ember Halls**, **Rooms cleared: 6/6**, level, XP and build. The upgrade choice from that last kill is withdrawn. A hero death anywhere shows **Defeated** with **Vanguard fell to a {enemy} in {room}** and the rooms cleared so far.
14. **Try again** is disabled for half a second, then tap it (rapidly is fine). Expect one reload into Ember Hall with 100 HP, Level 0, XP 0 and Sword 10 damage every 0.80s. Stopping and pressing Play again must give the same fresh state.
...
- Select `Data/Weapons/Weapon_GruntStrike.asset`: Damage **6**, Interval **1**. Set Damage to **40** and expect death on the third strike in Ember Hall and an immediate **Defeated** result with **Rooms cleared: 0/6** and no upgrades. Stop and restore **6**. Clearing `_weapon` on `Enemy_Grunt.asset` must throw `Room 0 wave 0 needs an enemy definition with a weapon and a prefab carrying Health, Targeting and AttackController.`
...
The implemented run is Descent floor 1: six rooms ending in a boss, with a forge choice, a victory or defeat result and a one-tap restart. Floor 2 with a modifier and scaling, Extract/Descend, additional enemies/weapons, escalating encounters, a boss, result screen, and in-game restart are deliberately still pending. The implementation plan assigns the next reward/choice slice to Day 3.
