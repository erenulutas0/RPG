...
## Two-sided enemy spacing — phone check pending — 2026-09-16

Development APK 24,739,335 bytes, SHA-256 `24F1FD800362ACBDFB518C8F86D78367E3068FDF11271BF1C071F1A3476CABDA`, built after the full gate. **Not installed.** The handoff recorded an active owner run on the S23, and a read-only state check (no input, no capture) found `com.cryptforge.prototype` in focus with the display awake and no keyguard, so installing would have ended whatever run was open. No tap, swipe, capture, install or profile change was made in this slice.

The device check to run once the owner hands over USB: install, confirm the stored profile and any open run are expendable or preserved, then on the proof floor (`development/start-floor.txt` holding `floor_density_proof`, removed afterwards) watch a standing and a drag-kiting fight for enemies overlapping, stepping back and forth or freezing in place, read the `[Perf]` lines for the steady-combat frame times with ten enemies walking, and return to Ember Hall 1/6. PlayMode already proves the scene keeps the spacing on every frame in the Editor; the phone runs IL2CPP, whose float arithmetic is a third runtime, so crowded fights there may end differently from the tables in `22` while keeping the same guarantees.
