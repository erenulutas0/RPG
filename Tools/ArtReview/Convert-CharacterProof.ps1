...
if ($Mode -eq 'Source') {
    ...
    if ($DaggersWeapon) { $rawFolder = Join-Path $conversionRoot 'TestResults/daggers-weapon-source'; New-Item -ItemType Directory -Force -Path $rawFolder | Out-Null; $sourceFiles = @('../2026-09-19/daggers-weapon-01/dagger-v1.png') }
