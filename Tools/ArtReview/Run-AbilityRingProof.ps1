...
$proofTarget = Join-Path $proofRoot 'Assets/_Project/Tests/PlayMode/AbilityRingCapture.cs'
$proofMeta = "$proofTarget.meta"
$proofResults = Join-Path $proofRoot 'TestResults/ability-ring.xml'
...
try {
    ...
    Copy-Item -LiteralPath "$PSScriptRoot/AbilityRingCapture.cs" -Destination $proofTarget
    ...
    $proofProcess = Start-Process -FilePath $UnityEditor -ArgumentList @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter','AbilityRingCapture','-testResults',"`"$proofResults`"",'-logFile',"`"$proofRoot/TestResults/ability-ring.log`"") -PassThru -WindowStyle Hidden
    $proofProcess.WaitForExit()
    if ($proofProcess.ExitCode -ne 0 -or !(Test-Path $proofResults)) { throw 'Unity capture failed; inspect TestResults/ability-ring.log.' }
    ...
    & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Renders -AbilityRing
    Write-Output "Ability ring capture: $($proofRun.passed) passed; no APK built or installed."
} finally {
