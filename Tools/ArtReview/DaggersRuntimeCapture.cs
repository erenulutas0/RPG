...
{
    public sealed class DaggersRuntimeCapture
    {
        private const string Output = "ArtDirection/2026-09-19/daggers-runtime-01/";
            ...
            foreach (string id in new[] { "weapon_daggers" })
            {
                ...
                foreach (var direction in new[] { new Vector2(1,1),new Vector2(-1,1),new Vector2(1,-1),new Vector2(-1,-1) })
                {
                    ...
                    for (int i = 0; i < 80; i++)
                    {
                        ...
                        if(!capturedCast && _look.IsSwinging && Quaternion.Angle(GameObject.Find("Dagger Right").transform.localRotation,Quaternion.identity)>36) { Detail(label+"-strike"); capturedCast=true; }
                    }
