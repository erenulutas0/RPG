...
{
    public sealed class GruntRuntimeCapture
    {
        private const string Output = "ArtDirection/2026-09-17/grunt-polish-01/";
        private Camera _camera;
        private Scene _scene;
        [UnityTest]
        public IEnumerator CaptureRuntimeGrunt()
        {
            TestProfile.Begin();
            ...
            {
                if (mite == null && look.Look == EnemyLook.Grunt && look.PaintedArt != null) mite = look;
            ...
            foreach (var direction in new[] { new Vector3(-1,-.5f), new Vector3(1,-.5f), new Vector3(-1,.5f), new Vector3(1,.5f) })
            {
                ...
                var target = new GameObject("Grunt capture target").AddComponent<Health>(); target.Initialize(1000);
            ...
            for (int i = 0; i < 16; i++) { if(i==0||i==2||i==6||i==12) Capture("runtime-hit-" + i.ToString("D2"),600,600); yield return null; }
            ...
            _camera.transform.position = new Vector3(0,.25f,-10); _camera.orthographicSize = .9f;
            Time.timeScale = 1;
            for (int i = 0; i < 26; i++) { if(i==0||i==6||i==14||i==25) Capture("runtime-death-" + i.ToString("D2"),600,600); yield return null; }
            LogAssert.NoUnexpectedReceived();
        }
        private void Detail(EnemyLookView mite,string name)
        {
            _camera.transform.position = new Vector3(mite.transform.position.x,mite.transform.position.y+.5f,-10);
        ...
        public IEnumerator Cleanup()
        {
            ...
            {
                SceneManager.SetActiveScene(SceneManager.CreateScene("Grunt capture cleanup"));
