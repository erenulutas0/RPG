#requires -Version 7
# Capture the integrated platform. -Reimport configures unchanged masters and the scene material reference.
...
function Invoke-PlatformUnity([string[]]$Arguments) {
    $process = Start-Process -FilePath $UnityEditor -ArgumentList $Arguments -PassThru -WindowStyle Hidden
    $process.WaitForExit()
    if ($process.ExitCode -ne 0) { throw "Unity failed: $($process.ExitCode). Inspect TestResults/platform-*.log." }
}
...
try {
    if ($Reimport) {
        ...
        $importTarget = "$proofRoot/Assets/_Project/Scripts/Editor/BuildPlatformMaterials.cs"
        Install-TemporarySource "$PSScriptRoot/BuildPlatformMaterials.cs" $importTarget
        try {
            Invoke-PlatformUnity @('-batchmode','-quit','-projectPath',"`"$proofRoot`"",'-executeMethod','Cryptforge.Editor.BuildPlatformMaterials.Build','-logFile',"`"$proofRoot/TestResults/platform-import.log`"")
        } finally { Remove-Item -LiteralPath $importTarget,"$importTarget.meta" }
    }
    $captureClass = 'PlatformRuntimeCapture'
    $captureFolder = 'platform-runtime-01'
    ...
    try {
        ...
        $result = "$proofRoot/TestResults/platform-capture.xml"
        if (Test-Path $result) { Remove-Item -LiteralPath $result }
        Invoke-PlatformUnity @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter',$captureClass,'-testResults',"`"$result`"",'-logFile',"`"$proofRoot/TestResults/platform-capture.log`"")
        $run = ([xml](Get-Content -LiteralPath $result)).'test-run'
        if ($run.result -ne 'Passed') { throw 'Platform capture assertions failed.' }
        Add-Type -AssemblyName System.Drawing
        Get-ChildItem -LiteralPath "$proofRoot/ArtDirection/2026-09-18/$captureFolder" -Filter '*.bmp' | ForEach-Object {
