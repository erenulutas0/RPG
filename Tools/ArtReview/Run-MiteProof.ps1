...
function Invoke-MiteUnity([string[]]$Arguments) {
    $process = Start-Process -FilePath $UnityEditor -ArgumentList $Arguments -PassThru -WindowStyle Hidden
    $process.WaitForExit()
    if ($process.ExitCode -ne 0) { throw "Unity failed: $($process.ExitCode). Inspect TestResults/mite-*.log." }
}
...
try {
    if ($Reimport) {
        & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Source -Mite
        $importTarget = "$proofRoot/Assets/_Project/Scripts/Editor/BuildMiteAssets.cs"
        Install-TemporarySource "$PSScriptRoot/BuildMiteAssets.cs" $importTarget
        try {
            Invoke-MiteUnity @('-batchmode','-quit','-projectPath',"`"$proofRoot`"",'-executeMethod','Cryptforge.Editor.BuildMiteAssets.Build','-logFile',"`"$proofRoot/TestResults/mite-import.log`"")
        } finally { Remove-Item -LiteralPath $importTarget,"$importTarget.meta" }
    }
    $captureClass = 'MiteRuntimeCapture'
    $captureFolder = 'mite-runtime-01'
    ...
    try {
        ...
        $result = "$proofRoot/TestResults/mite-capture.xml"
        if (Test-Path $result) { Remove-Item -LiteralPath $result }
        Invoke-MiteUnity @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter',$captureClass,'-testResults',"`"$result`"",'-logFile',"`"$proofRoot/TestResults/mite-capture.log`"")
        $run = ([xml](Get-Content -LiteralPath $result)).'test-run'
        if ($run.result -ne 'Passed') { throw 'Mite capture assertions failed.' }
