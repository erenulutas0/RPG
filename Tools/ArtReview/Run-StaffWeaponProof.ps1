...
$proofTarget = Join-Path $proofRoot 'Assets/_Project/Tests/PlayMode/StaffWeaponCapture.cs'
$proofMeta = "$proofTarget.meta"
$proofResults = Join-Path $proofRoot 'TestResults/staff-weapon.xml'
...
try {
    ...
    Copy-Item -LiteralPath "$PSScriptRoot/StaffWeaponCapture.cs" -Destination $proofTarget
    ...
    $proofProcess = Start-Process -FilePath $UnityEditor -ArgumentList @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter','StaffWeaponCapture','-testResults',"`"$proofResults`"",'-logFile',"`"$proofRoot/TestResults/staff-weapon.log`"") -PassThru -WindowStyle Hidden
    $proofProcess.WaitForExit()
    if ($proofProcess.ExitCode -ne 0 -or !(Test-Path $proofResults)) { throw 'Unity capture failed; inspect TestResults/staff-weapon.log.' }
    ...
    & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Renders -StaffWeapon
    Write-Output "Staff weapon capture: $($proofRun.passed) passed; no APK built or installed."
} finally {
