...
$proofTarget = Join-Path $proofRoot 'Assets/_Project/Tests/PlayMode/StaffSplashRuntimeCapture.cs'
$proofMeta = "$proofTarget.meta"
$proofResults = Join-Path $proofRoot 'TestResults/staff-runtime.xml'
...
try {
    ...
    Copy-Item -LiteralPath "$PSScriptRoot/StaffSplashRuntimeCapture.cs" -Destination $proofTarget
    ...
    $proofProcess = Start-Process -FilePath $UnityEditor -ArgumentList @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter','StaffSplashRuntimeCapture','-testResults',"`"$proofResults`"",'-logFile',"`"$proofRoot/TestResults/staff-runtime.log`"") -PassThru -WindowStyle Hidden
    $proofProcess.WaitForExit()
    if ($proofProcess.ExitCode -ne 0 -or !(Test-Path $proofResults)) { throw 'Unity capture failed; inspect TestResults/staff-runtime.log.' }
    ...
    & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Renders -StaffSplashRuntime
