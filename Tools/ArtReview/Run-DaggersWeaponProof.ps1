...
$proofTarget = Join-Path $proofRoot 'Assets/_Project/Tests/PlayMode/DaggersWeaponCapture.cs'
$proofMeta = "$proofTarget.meta"
$proofResults = Join-Path $proofRoot 'TestResults/daggers-weapon.xml'
...
try {
    ...
    Copy-Item -LiteralPath "$PSScriptRoot/DaggersWeaponCapture.cs" -Destination $proofTarget
    ...
    $proofProcess = Start-Process -FilePath $UnityEditor -ArgumentList @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter','DaggersWeaponCapture','-testResults',"`"$proofResults`"",'-logFile',"`"$proofRoot/TestResults/daggers-weapon.log`"") -PassThru -WindowStyle Hidden
    $proofProcess.WaitForExit()
    if ($proofProcess.ExitCode -ne 0 -or !(Test-Path $proofResults)) { throw 'Unity capture failed; inspect TestResults/daggers-weapon.log.' }
    ...
    & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Renders -DaggersWeapon
    Write-Output "Daggers weapon capture: $($proofRun.passed) passed; no APK built or installed."
} finally {
