...
$proofTarget = Join-Path $proofRoot 'Assets/_Project/Tests/PlayMode/AbilityPulseCapture.cs'
$proofMeta = "$proofTarget.meta"
$proofResults = Join-Path $proofRoot 'TestResults/ability-pulse.xml'
...
try {
    ...
    Copy-Item -LiteralPath "$PSScriptRoot/AbilityPulseCapture.cs" -Destination $proofTarget
    ...
    $proofProcess = Start-Process -FilePath $UnityEditor -ArgumentList @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter','AbilityPulseCapture','-testResults',"`"$proofResults`"",'-logFile',"`"$proofRoot/TestResults/ability-pulse.log`"") -PassThru -WindowStyle Hidden
    $proofProcess.WaitForExit()
    if ($proofProcess.ExitCode -ne 0 -or !(Test-Path $proofResults)) { throw 'Unity capture failed; inspect TestResults/ability-pulse.log.' }
    ...
    & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Renders -AbilityPulse
    Write-Output "Ability pulse capture: $($proofRun.passed) passed; no APK built or installed."
} finally {
