...
$proofTarget = Join-Path $proofRoot 'Assets/_Project/Tests/PlayMode/DaggersRuntimeCapture.cs'
$proofMeta = "$proofTarget.meta"
$proofResults = Join-Path $proofRoot 'TestResults/daggers-runtime.xml'
...
try {
    ...
    Copy-Item -LiteralPath "$PSScriptRoot/DaggersRuntimeCapture.cs" -Destination $proofTarget
    ...
    $proofProcess = Start-Process -FilePath $UnityEditor -ArgumentList @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter','DaggersRuntimeCapture','-testResults',"`"$proofResults`"",'-logFile',"`"$proofRoot/TestResults/daggers-runtime.log`"") -PassThru -WindowStyle Hidden
    $proofProcess.WaitForExit()
    if ($proofProcess.ExitCode -ne 0 -or !(Test-Path $proofResults)) { throw 'Unity capture failed; inspect TestResults/daggers-runtime.log.' }
    ...
    & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Renders -DaggersRuntime
    Write-Output "Daggers runtime capture: $($proofRun.passed) passed; no APK built or installed."
} finally {
