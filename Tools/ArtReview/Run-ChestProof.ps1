...
$proofTarget = Join-Path $proofRoot 'Assets/_Project/Tests/PlayMode/ChestProofCapture.cs'
$proofMeta = "$proofTarget.meta"
$proofResults = Join-Path $proofRoot 'TestResults/chest-proof.xml'
...
try {
    ...
    Copy-Item -LiteralPath "$PSScriptRoot/ChestProofCapture.cs" -Destination $proofTarget
    ...
    $proofProcess = Start-Process -FilePath $UnityEditor -ArgumentList @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter','ChestProofCapture','-testResults',"`"$proofResults`"",'-logFile',"`"$proofRoot/TestResults/chest-proof.log`"") -PassThru -WindowStyle Hidden
    $proofProcess.WaitForExit()
    if ($proofProcess.ExitCode -ne 0 -or !(Test-Path $proofResults)) { throw 'Unity capture failed; inspect TestResults/chest-proof.log.' }
    ...
    & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Renders -Chest
    Write-Output "Chest proof capture: $($proofRun.passed) passed; no APK built or installed."
} finally {
