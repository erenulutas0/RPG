...
$staffTemp=Join-Path $staffRoot 'Assets/_Project/Scripts/Editor/BuildStaffAsset.cs'
...
try {
    & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Source -StaffWeapon
    Copy-Item -LiteralPath "$PSScriptRoot/BuildStaffAsset.cs" -Destination $staffTemp
    ...
    $staffProcess=Start-Process -FilePath $UnityEditor -ArgumentList @('-batchmode','-quit','-projectPath',"`"$staffRoot`"",'-executeMethod','Cryptforge.Editor.BuildStaffAsset.Build','-logFile',"`"$staffRoot/TestResults/staff-import.log`"") -PassThru -WindowStyle Hidden
