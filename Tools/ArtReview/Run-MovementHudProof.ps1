...
$proofTarget = Join-Path $proofRoot 'Assets/_Project/Tests/PlayMode/MovementHudCapture.cs'
$proofMeta = "$proofTarget.meta"
$proofResults = Join-Path $proofRoot 'TestResults/movement-hud.xml'
...
try {
    ...
    Copy-Item -LiteralPath "$PSScriptRoot/MovementHudCapture.cs" -Destination $proofTarget
    ...
    $proofProcess = Start-Process -FilePath $UnityEditor -ArgumentList @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter','MovementHudCapture','-testResults',"`"$proofResults`"",'-logFile',"`"$proofRoot/TestResults/movement-hud.log`"") -PassThru -WindowStyle Hidden
    $proofProcess.WaitForExit()
    if ($proofProcess.ExitCode -ne 0 -or !(Test-Path $proofResults)) { throw 'Unity capture failed; inspect TestResults/movement-hud.log.' }
    ...
    & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Renders -MovementHud
    Write-Output "Movement HUD capture: $($proofRun.passed) passed; no APK built or installed."
} finally {
