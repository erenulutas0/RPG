...
$proofResults = Join-Path $proofRoot 'TestResults/character-proof.xml'
...
try {
    & (Join-Path $PSScriptRoot 'Convert-CharacterProof.ps1') -Mode Source
    ...
    $proofProcess = Start-Process -FilePath $UnityEditor -ArgumentList @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter','CharacterProofCapture','-testResults',"`"$proofResults`"",'-logFile',"`"$(Join-Path $proofRoot 'TestResults/character-proof.log')`"") -PassThru -WindowStyle Hidden
    $proofProcess.WaitForExit()
    if ($proofProcess.ExitCode -ne 0 -or !(Test-Path $proofResults)) { throw 'Unity capture did not finish successfully; inspect TestResults/character-proof.log.' }
    ...
    & (Join-Path $PSScriptRoot 'Convert-CharacterProof.ps1') -Mode Renders
    Write-Output "Character proof: $($proofRun.passed) passed. Outputs: ArtDirection/2026-09-16/character-unity-01"
} finally {
