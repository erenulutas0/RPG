...
$proofTarget = Join-Path $proofRoot 'Assets/_Project/Tests/PlayMode/AbilityRingRuntimeCapture.cs'
$proofMeta = "$proofTarget.meta"
$proofResults = Join-Path $proofRoot 'TestResults/ability-ring-runtime.xml'
...
try {
    ...
    Copy-Item -LiteralPath "$PSScriptRoot/AbilityRingRuntimeCapture.cs" -Destination $proofTarget
    ...
    $proofProcess = Start-Process -FilePath $UnityEditor -ArgumentList @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter','AbilityRingRuntimeCapture','-testResults',"`"$proofResults`"",'-logFile',"`"$proofRoot/TestResults/ability-ring-runtime.log`"") -PassThru -WindowStyle Hidden
    $proofProcess.WaitForExit()
    if ($proofProcess.ExitCode -ne 0 -or !(Test-Path $proofResults)) { throw 'Unity capture failed; inspect TestResults/ability-ring-runtime.log.' }
    ...
    & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Renders -AbilityRingRuntime
    Write-Output "Ability ring runtime capture: $($proofRun.passed) passed; no APK built or installed."
} finally {
