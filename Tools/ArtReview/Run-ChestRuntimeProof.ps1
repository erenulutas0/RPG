...
$proofTarget = Join-Path $proofRoot 'Assets/_Project/Tests/PlayMode/ChestRuntimeCapture.cs'
$proofMeta = "$proofTarget.meta"
$proofResults = Join-Path $proofRoot 'TestResults/chest-runtime.xml'
...
try {
    ...
    Copy-Item -LiteralPath "$PSScriptRoot/ChestRuntimeCapture.cs" -Destination $proofTarget
    ...
    $proofProcess = Start-Process -FilePath $UnityEditor -ArgumentList @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter','ChestRuntimeCapture','-testResults',"`"$proofResults`"",'-logFile',"`"$proofRoot/TestResults/chest-runtime.log`"") -PassThru -WindowStyle Hidden
    $proofProcess.WaitForExit()
    if ($proofProcess.ExitCode -ne 0 -or !(Test-Path $proofResults)) { throw 'Unity capture failed; inspect TestResults/chest-runtime.log.' }
    ...
    & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Renders -ChestRuntime
    Write-Output "Chest runtime capture: $($proofRun.passed) passed; no APK built or installed."
} finally {
