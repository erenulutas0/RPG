...
$proofTarget = Join-Path $proofRoot 'Assets/_Project/Tests/PlayMode/GruntMotionCapture.cs'
$proofMeta = "$proofTarget.meta"
$proofResults = Join-Path $proofRoot 'TestResults/grunt-motion.xml'
...
try {
    & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Source -GruntMotion
    Copy-Item -LiteralPath "$PSScriptRoot/GruntMotionCapture.cs" -Destination $proofTarget
    ...
    $proofProcess = Start-Process -FilePath $UnityEditor -ArgumentList @('-batchmode','-projectPath',"`"$proofRoot`"",'-runTests','-testPlatform','PlayMode','-testFilter','GruntMotionCapture','-testResults',"`"$proofResults`"",'-logFile',"`"$proofRoot/TestResults/grunt-motion.log`"") -PassThru -WindowStyle Hidden
    $proofProcess.WaitForExit()
    if ($proofProcess.ExitCode -ne 0 -or !(Test-Path $proofResults)) { throw 'Unity capture failed; inspect TestResults/grunt-motion.log.' }
    ...
    & "$PSScriptRoot/Convert-CharacterProof.ps1" -Mode Renders -GruntMotion
    Write-Output "Grunt motion capture: $($proofRun.passed) passed; no APK built or installed."
} finally {
