...
namespace Cryptforge.Combat
{
    ...
    public static class EntrySides
    {
        ...
        // corner forms up its enemies with PackLayout, entryDepth from the hero plus their formation depth. A corner with
        // an enemy that would start off the platform, or inside the margin the hero keeps from its rim, closes for this
        // wave and the open corners take its enemies, so a hero at the rim is approached from where the platform is; with
        // the hero near the centre every corner stays open. Should every corner close, on a platform too small for the
        // wave, each enemy that starts outside is drawn in toward the hero until it stands inside.
        ...
        // The point on the line from the hero to the placement nearest the placement that stands inside the rim's margin.
        private static EntryPlacement DrawnInside(EntryPlacement placement, float heroX, float heroY, ArenaGeometry platform)
        {
            ...
            for (int i = 0; i < 24; i++)
            {
                ...
                if (platform.IsOnPlatform(heroX + (placement.X - heroX) * middle, heroY + (placement.Y - heroY) * middle, HeroMotion.EdgeMargin))
                    inside = middle;
                else
                    outside = middle;
            }
